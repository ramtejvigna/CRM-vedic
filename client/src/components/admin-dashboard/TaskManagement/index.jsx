...
const TaskManagement = () => {
  ...
  const fetchTasks = async (page = 1) => {
    ...
    try {
      const response = await axios.get(`https://vedic-backend.netlify.app/api/tasks?page=${page}&limit=${tasksPerPage}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (selectedTask) {
        await axios.put(
          `https://vedic-backend.netlify.app/api/tasks/${selectedTask._id}`,
          newTask
        );
        enqueueSnackbar("Task updated successfully", { variant: "success" });
      } else {
        await axios.post("https://vedic-backend.netlify.app/api/tasks", newTask);
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(`https://vedic-backend.netlify.app/api/tasks/${taskToDelete}`);
      fetchTasks(currentPage);
  ...
  const handleAddComment = async (id) => {
    ...
    try {
      await axios.post(`https://vedic-backend.netlify.app/api/tasks/${id}/comment`, {
        text: newComment,
